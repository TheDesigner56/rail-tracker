...
const { findTrains, trackTrain } = require('./node_modules/trainspy/dist/src/index.js');
...
// ── Station database (from trainspy — 2,500+ stations) ──
const RAW_STATIONS = require('./node_modules/trainspy/dist/src/map/stationCodes.json');
const STATIONS = {};
...
const STATION_LOCS = require('./node_modules/trainspy/dist/src/map/stationLocations.json');
...
app.get('/api/arrivals/:crs', async (req, res) => {
  ...
  try {
    const raw = await findTrains(crs);
    // Filter to services arriving at this station
    const arrivals = (raw.departures || []).filter(t => {
      const dest = (t.destination || '').toLowerCase();
      const stationName = (STATIONS[crs] || '').toLowerCase();
    ...
    console.error(`Error fetching arrivals ${crs}:`, err.message);
    res.status(502).json({ error: `Failed to fetch arrivals for ${crs}` });
  }
});

// Station search (fuzzy)
...
function renderStation(crs, name, data, mode = 'departures') {
  ...
  const otherMode = isArrivals ? 'departures' : 'arrivals';
  const otherLabel = isArrivals ? 'Departures' : 'Arrivals';
  const otherUrl = isArrivals ? `/station/${crs}` : `/station/${crs}/arrivals`;
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`Rail Tracker v1.0.0 running on http://0.0.0.0:${PORT}`);
