...
.mappin{width:14px;height:14px;border-radius:50%;border:2px solid #fff}
.leaflet-tooltip.pinlab{background:#fff;border:none;border-radius:8px;box-shadow:0 2px 7px rgba(0,0,0,.28);font-weight:700;font-size:.72rem;padding:2px 8px;color:var(--ink)}
...
function dotIcon(c){return L.divIcon({className:'',iconSize:[16,16],iconAnchor:[8,8],html:'<div class="mappin" style="background:'+c+';box-shadow:0 0 0 6px '+c+'2e,0 1px 4px rgba(0,0,0,.4)"></div>'})}
...
function load(){
 ...
 function refresh(){
  ...
  if(z>=8){var n=0;for(var i=0;i<ALL.length&&n<700;i++){var s=ALL[i];if(majorCodes[s.code]||!b.contains([s.lat,s.lon]))continue;n++;
   L.circleMarker([s.lat,s.lon],{renderer:canvas,radius:z>=11?4:3,color:'#fff',weight:1,fillColor:'#5B6472',fillOpacity:.95}).on('click',go(s.code)).bindTooltip('<div class="sc-h" style="margin-bottom:3px"><b>'+s.code+'</b> '+s.name+'</div><div class="sc-go">View live times →</div>',{direction:'top',className:'stncard',offset:[0,-4]}).addTo(secLayer);}}
  // Labels: major first (priority), then secondary at higher zoom, collision-avoided.
  labelLayer.clearLayers();var placed=[];
  function lab(s,cls){if(s.lat==null)return;var p=map.latLngToLayerPoint([s.lat,s.lon]);if(placed.some(function(q){return Math.abs(q.x-p.x)<54&&Math.abs(q.y-p.y)<22}))return;placed.push(p);L.tooltip({permanent:true,direction:'bottom',offset:[0,11],className:cls,interactive:false}).setLatLng([s.lat,s.lon]).setContent(s.code).addTo(labelLayer);}
  MAJOR.forEach(function(s){lab(s,'pinlab')});
  if(z>=10){var m=0;for(var j=0;j<ALL.length&&m<400;j++){var s2=ALL[j];if(majorCodes[s2.code]||!b.contains([s2.lat,s2.lon]))continue;m++;lab(s2,'pinlab sec')}}
 }
