...
function renderHome() {
  const head = `
.hero-j{padding:30px 0 16px}
.hero-j h1{font-size:2rem;font-weight:800;letter-spacing:-.02em}
.hero-j p{color:var(--ink2);margin-top:6px;font-size:.95rem}
.planner{background:var(--card);border-radius:18px;padding:12px;max-width:580px;display:flex;flex-direction:column;gap:10px}
.pl-fields{position:relative;display:flex;flex-direction:column;gap:8px}
.pl-field{position:relative;display:flex;align-items:center;gap:9px;background:#fff;border:1px solid var(--line);border-radius:12px;padding:0 12px}
.pl-field:focus-within{border-color:var(--blue)}
.pl-field input{flex:1;border:none;outline:none;padding:14px 0;font-size:1rem;background:transparent;color:var(--ink);min-width:0}
.pl-field #locate{background:none;border:none;cursor:pointer;font-size:1.15rem;color:var(--blue);padding:4px}
.pl-swap{position:absolute;right:12px;top:calc(50% - 16px);width:32px;height:32px;border-radius:50%;background:#fff;border:1px solid var(--line);cursor:pointer;display:grid;place-items:center;box-shadow:0 1px 4px rgba(0,0,0,.12);z-index:2}
.pl-swap:hover{background:var(--soft)}
.pl-go{width:100%;justify-content:center;padding:13px}
.ac{position:absolute;top:calc(100% + 4px);left:0;right:0;background:#fff;border:1px solid var(--line);border-radius:12px;box-shadow:0 10px 26px rgba(0,0,0,.14);z-index:30;max-height:300px;overflow-y:auto;display:none;padding:4px}
...
.loading2{padding:24px;text-align:center;color:var(--ink3)}
.jres{margin:20px 0}
.jres-h{font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--ink3);margin-bottom:10px}
.jrow{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px;border:1px solid var(--line);border-radius:14px;margin-bottom:8px;background:#fff}
.jrow:hover{background:var(--soft2)}
.j-times{display:flex;align-items:center;gap:8px;font-size:1.2rem;font-weight:700}
.j-times .ar{color:var(--ink3);font-size:.9rem}
.j-times .t.late{color:var(--amber)}.j-times .t.bad{color:var(--red);text-decoration:line-through}
.j-sub{font-size:.8rem;color:var(--ink2);margin-top:4px;display:flex;align-items:center;gap:6px;flex-wrap:wrap}
.j-r{text-align:right;flex-shrink:0}.j-dur{font-weight:700;font-size:.95rem}.j-badge{font-size:.7rem;color:var(--green);font-weight:700;margin-top:2px}
...
.chip{display:inline-flex;align-items:center;gap:7px;padding:9px 13px;border:1px solid var(--line);border-radius:999px;font-size:.85rem;background:#fff;color:var(--ink);cursor:pointer}
.chip:hover{background:var(--soft2)}
@media(max-width:560px){.hero-j h1{font-size:1.6rem}.j-times{font-size:1.05rem}}`;
  const body = `${nav()}<div class="wrap">
  <div class="hero-j"><h1>Where to?</h1><p>Plan a live journey from your nearest station to anywhere on the GB rail network.</p></div>
  <div class="planner">
    <div class="pl-fields">
  ...
  <div class="sect"><div class="sect-title">Live network</div><div class="chips">
    <a class="chip" href="/disruptions">⚠️ Disruptions board</a><a class="chip" href="/map">🗺 Network map</a><a class="chip" href="/operators">🚆 Operators</a>
  </div></div>
  ${footer()}
</div>`;
  const script = `(function(){
 var fromCode=null,toCode=null;
 ...
 function render(d){
  if(!d.services||!d.services.length){res.innerHTML='<div class="jres"><div class="jres-h">'+(d.fromName||fromCode)+' → '+(d.toName||toCode)+'</div><p class="muted" style="padding:4px 2px">No direct trains found in the next couple of hours — you may need to change. Try the <a href="/station/'+fromCode+'/departures" style="color:var(--blue);font-weight:600">'+(d.fromName||fromCode)+' departures board</a>.</p></div>';return}
  document.getElementById('from-input').value=d.fromName||fromCode;document.getElementById('to-input').value=d.toName||toCode;
  res.innerHTML='<div class="jres"><div class="jres-h">'+d.fromName+' → '+d.toName+' · next direct trains</div>'+d.services.map(function(s){
   var cls=s.cancelled?'bad':(s.depDelay>5||s.arrDelay>5)?'late':'';
   var stops=s.intermediateStops===0?'Non-stop':(s.intermediateStops+' stop'+(s.intermediateStops>1?'s':''));
   return '<a class="jrow" href="/service/'+s.id+'"><div class="j-l"><div class="j-times"><span class="t '+cls+'">'+s.dep+'</span><span class="ar">→</span><span class="t '+cls+'">'+s.arr+'</span></div>'+
    '<div class="j-sub"><span class="oplogo" style="background:'+s.color+'">'+s.operatorCode+'</span> '+s.operator+' · Plat '+(s.platform||'—')+' · '+stops+(s.cancelled?' · <b class="red">Cancelled</b>':'')+'</div></div>'+
    '<div class="j-r"><div class="j-dur">'+(s.duration!=null?fmtDur(s.duration):'')+'</div><div class="j-badge">Direct</div></div></a>';
  }).join('')+'</div>';
 }
 function search(){if(!fromCode||!toCode){res.innerHTML='<p class="muted" style="padding:14px 2px">Choose a station to travel from and to.</p>';return}
  history.replaceState(null,'','/?from='+fromCode+'&to='+toCode);
  res.innerHTML='<div class="loading2">Finding trains…</div>';
  fetch('/api/journey?from='+fromCode+'&to='+toCode).then(function(r){return r.json()}).then(render).catch(function(){res.innerHTML='<p class="muted" style="padding:14px 2px">Couldn\\'t plan that journey. Please try again.</p>'})}
 ...
 function locate(){if(!navigator.geolocation)return;var fi=document.getElementById('from-input');fi.placeholder='📍 Locating…';
  navigator.geolocation.getCurrentPosition(function(p){fetch('/api/nearby?lat='+p.coords.latitude+'&lon='+p.coords.longitude+'&n=6').then(function(r){return r.json()}).then(function(list){fi.placeholder='From station';if(!list||!list.length)return;
    if(!fromCode){fromCode=list[0].code;fi.value=list[0].name}
    var nb=document.getElementById('nearby');nb.innerHTML=list.map(function(s){return '<button class="chip" data-code="'+s.code+'" data-name="'+s.name.replace(/"/g,'')+'">'+s.code+' · '+s.name+' · '+s.distKm.toFixed(1)+'km</button>'}).join('');
    document.getElementById('nearby-sect').style.display='';
    nb.querySelectorAll('.chip').forEach(function(b){b.addEventListener('click',function(){fromCode=b.dataset.code;fi.value=b.dataset.name;if(toCode)search()})});
    if(toCode)search();
 ...
 if(qf&&qt){fromCode=qf;toCode=qt;document.getElementById('from-input').value=qf;document.getElementById('to-input').value=qt;search()}else{locate()}
  ...
  return shell({ title: 'Rail Tracker — Plan a Live UK Train Journey', desc: 'Plan a live UK rail journey from your nearest station: direct trains, times, platforms and live tracking. Find how to get from A to B by train.', head, body, script });
}
