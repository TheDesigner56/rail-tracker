...
// Service worker (push notifications) + PWA manifest, served from the origin root.
const SW_JS = `self.addEventListener('push',function(e){var d={};try{d=e.data?e.data.json():{}}catch(x){}var t=d.title||'Rail Tracker';e.waitUntil(self.registration.showNotification(t,{body:d.body||'',tag:d.tag||'rt',data:{url:d.url||'/trips'},renotify:true}))});
self.addEventListener('notificationclick',function(e){e.notification.close();var u=(e.notification.data&&e.notification.data.url)||'/trips';e.waitUntil(clients.matchAll({type:'window',includeUncontrolled:true}).then(function(cl){for(var i=0;i<cl.length;i++){if(cl[i].url.indexOf(u)>-1&&'focus'in cl[i])return cl[i].focus()}if(clients.openWindow)return clients.openWindow(u)}))});`;
const MANIFEST_JSON = JSON.stringify({
  name: 'Rail Tracker', short_name: 'Rail Tracker', start_url: '/', scope: '/', display: 'standalone',
  background_color: '#0E1014', theme_color: '#0E1014',
  icons: [{ src: "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><rect width='100' height='100' rx='22' fill='%232563EB'/><text y='.86em' x='15' font-size='62'>🚆</text></svg>", sizes: 'any', type: 'image/svg+xml' }],
});
...
function owner(){try{var k=localStorage.getItem('rt_owner');if(!k){k=(window.crypto&&crypto.randomUUID)?crypto.randomUUID():String(Date.now())+Math.random().toString(16).slice(2);localStorage.setItem('rt_owner',k)}return k}catch(e){return 'anon'}}
function b64u(s){var pad='='.repeat((4-s.length%4)%4);var b=(s+pad).replace(/-/g,'+').replace(/_/g,'/');var raw=atob(b);var a=new Uint8Array(raw.length);for(var i=0;i<raw.length;i++)a[i]=raw.charCodeAt(i);return a}
function enablePush(cb){
 if(!('serviceWorker' in navigator)||!('PushManager' in window)){alert('Notifications need a modern browser. On iPhone, add Rail Tracker to your Home Screen first (Share → Add to Home Screen), then open it from there and try again.');return cb&&cb(false)}
 Notification.requestPermission().then(function(perm){if(perm!=='granted')return cb&&cb(false);
  navigator.serviceWorker.register('/sw.js').then(function(reg){return fetch('/api/push/vapid').then(function(r){return r.json()}).then(function(v){if(!v.key)return cb&&cb(false);
   return reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:b64u(v.key)}).then(function(sub){return fetch('/api/push/subscribe',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({owner:owner(),subscription:sub.toJSON?sub.toJSON():sub})}).then(function(){cb&&cb(true)})})})}).catch(function(){cb&&cb(false)})})}
...
function footer() {
  return `<footer class="footer"><div class="fl"><a href="/">Plan</a><a href="/trips">My trips</a><a href="/disruptions">Disruptions</a><a href="/map">Map</a><a href="/operators">Operators</a><a href="/about">About</a><a href="https://github.com/TheDesigner56/rail-tracker">GitHub</a></div>
...
<link rel="preconnect" href="https://api.maptiler.com" crossorigin><link rel="preconnect" href="https://unpkg.com" crossorigin>
...
<link rel="manifest" href="/manifest.webmanifest"><link rel="apple-touch-icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><rect width='100' height='100' rx='22' fill='%232563EB'/><text y='.86em' x='15' font-size='62'>🚆</text></svg>"><meta name="apple-mobile-web-app-capable" content="yes"><meta name="apple-mobile-web-app-title" content="Rail Tracker">
...
.track-btn{width:100%;justify-content:center;margin:6px 0 4px;background:var(--blue);color:#fff;border-color:var(--blue);font-weight:700}
.track-btn:hover{filter:brightness(.96)}.track-btn:disabled{opacity:.6}
...
@media(max-width:560px){.sh-title{font-size:1.35rem}.plan-times{font-size:1.1rem}#map{height:60vh}}`;
  ...
  <div class="search-hero"><div class="sh-card">
    ...
    <div class="pl-fields">
      <div class="pl-field"><span>📍</span><input id="from-input" type="text" placeholder="From station" autocomplete="off" aria-label="From station"><button id="locate" title="Use my location" aria-label="Use my location">⌖</button><div class="ac" id="from-ac"></div></div>
  ...
  <div class="panel"><div class="panel-inner">
    ...
    <div class="sect"><div class="sect-title">Live network</div><div class="chips"><a class="chip" href="/trips">🔔 My trips</a><a class="chip" href="/disruptions">⚠️ Disruptions board</a><a class="chip" href="/map">🗺 Network map</a><a class="chip" href="/operators">🚆 Operators</a></div></div>
  </div></div>
  ${footer()}
</div>`;
  const script = `
...
function renderRecent(){var a=loadRecent(),sec=document.getElementById('recent-sect'),box=document.getElementById('recent');if(!sec||!box)return;
 ...
 box.innerHTML=a.map(function(r){return '<span class="chip rc"><button class="rc-go" data-f="'+r.f+'" data-t="'+r.t+'" data-fn="'+r.fn+'" data-tn="'+r.tn+'">🕘 '+r.fn+' <span class="rc-ar">→</span> '+r.tn+'</button><button class="rc-x" data-f="'+r.f+'" data-t="'+r.t+'" title="Remove" aria-label="Remove recent journey">✕</button></span>'}).join('');
...
var JP=null,JM=null,JF=null,JT=null,JSEL=0;
function pickTrain(i){var p=JP&&JP[i];if(!p)return;JSEL=i;
...
function trackTrip(btn){var p=JP&&JP[JSEL];if(!p)return;
 var t=new Date(),def=t.getFullYear()+'-'+pad2(t.getMonth()+1)+'-'+pad2(t.getDate());
 var date=prompt('What date is this trip? (YYYY-MM-DD)',def);if(!date)return;if(!/^\\d{4}-\\d{2}-\\d{2}$/.test(date)){alert('Please use the format YYYY-MM-DD.');return}
 var legs=p.legs.map(function(l){return {mode:l.mode,line:l.line||null,from:l.from||null,fromCode:l.fromCode||null,to:l.to||null,toCode:l.toCode||null,dep:l.dep||null,arr:l.arr||null,serviceId:l.serviceId||null,operator:l.operator||null,operatorCode:l.operatorCode||null,color:l.color||null}});
 if(btn){btn.disabled=true;btn.textContent='Saving…'}
 fetch('/api/trips',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({owner:owner(),name:(JF||'')+' → '+(JT||''),travel_date:date,origin:JF||'',destination:JT||'',depart:p.depart,arrive:p.arrive,legs:legs})}).then(function(r){return r.json()}).then(function(res){
  if(res&&res.ok){enablePush(function(){location.href='/trips'})}
  else{alert('Couldn\\'t save the trip — the alerts backend may not be configured yet.');if(btn){btn.disabled=false;btn.textContent='🔔 Track this trip & get alerts'}}
 }).catch(function(){alert('Couldn\\'t save the trip right now.');if(btn){btn.disabled=false;btn.textContent='🔔 Track this trip & get alerts'}})}
...
function render(d){var plans=d.plans||[];var head=(d.fromName||fromCode)+' → '+(d.toName||toCode);
 ...
 JP=plans;JM=d.mode;JF=d.fromName||fromCode;JT=d.toName||toCode;JSEL=0;
 ...
 html+='<div id="itin">'+itinHTML(plans[0],d.mode,0)+'</div><button class="btn track-btn" onclick="trackTrip(this)">🔔 Track this trip &amp; get alerts</button>'+buyHTML(d)+'</div>';
 res.innerHTML=html;
...
function search(){if(!fromCode||!toCode){res.innerHTML='<p class="muted" style="padding:6px 2px">Choose where you\\'re travelling from and to.</p>';return}
 ...
 var go=document.getElementById('go');if(go){go.disabled=true;go.textContent='Finding trains…'}
 function done(){if(go){go.disabled=false;go.textContent='Find trains'}}
 fetch('/api/plan?from='+fromCode+'&to='+toCode).then(function(r){return r.json()}).then(function(d){done();render(d)}).catch(function(){done();res.innerHTML='<p class="muted" style="padding:6px 2px">Couldn\\'t plan that journey. Please try again.</p>'})}
...
document.getElementById('from-input').addEventListener('keydown',function(e){if(e.key==='Enter'){var ti=document.getElementById('to-input');if(ti.value.trim())ensure(search);else ti.focus()}});
...
var ld=document.createElement('script');ld.type='application/ld+json';ld.textContent=JSON.stringify({'@context':'https://schema.org','@type':'WebApplication',name:'Rail Tracker',url:location.origin+'/',applicationCategory:'TravelApplication',operatingSystem:'Web',description:'Live UK train times, door-to-door journey planning and real-time train tracking for every station in Great Britain.',offers:{'@type':'Offer',price:'0',priceCurrency:'GBP'}});document.head.appendChild(ld);`;
...
${footer()}</div>`;
  ...
  const script = `(function(){var tg=document.querySelectorAll('#wx .seg span');if(tg.length){
   ...
   (function(){var due=false;setTimeout(function(){if(document.visibilityState==='visible')location.reload();else due=true},90000);
    document.addEventListener('visibilitychange',function(){if(due&&document.visibilityState==='visible')location.reload()})})();`;
...
<div class="card plain" style="padding:8px 14px">
  ...
  // Auto-refresh only the live "now" view, and only while the tab is visible —
  // a hidden tab defers the reload until the user returns.
  const script = `(function(){if(new URLSearchParams(location.search).get('at'))return;var due=false;
setTimeout(function(){if(document.visibilityState==='visible')location.reload();else due=true},60000);
document.addEventListener('visibilitychange',function(){if(due&&document.visibilityState==='visible')location.reload()})})();`;
...
<p>Live running information comes from National Rail / Network Rail feeds via the <a href="https://www.realtimetrains.co.uk/">Realtime Trains</a> API. Cross-London and last-mile routing, line status and disruption messages are powered by <a href="https://tfl.gov.uk/">Transport for London</a> open data. Maps by <a href="https://www.maptiler.com/">MapTiler</a> and <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors; weather by <a href="https://open-meteo.com/">Open-Meteo</a>. Rail Tracker is an independent project and is not affiliated with, or endorsed by, National Rail, Network Rail, Transport for London or any train operating company. Times and platforms can change at short notice — always check station announcements before travelling.</p>
<h2>Privacy</h2>
<p>No accounts, no advertising trackers, no analytics cookies. Your recent searches and preferences (like dark mode) stay in your browser. If you save a trip for alerts, the journey details and an anonymous device token are stored so we can send the notifications you asked for — remove the trip and they go with it. Your live location is used only on your device (to find nearby stations and show you on the map) and is never sent to our servers.</p>
...
// ── My Trips — saved trips + proactive alert opt-in ────────────────────────
function renderTrips() {
  const head = `
.th{padding:22px 0 6px}.th h1{font-size:1.7rem;font-weight:800;letter-spacing:-.02em}
.tcard{border:1px solid var(--line);border-radius:16px;padding:15px 16px;margin-bottom:12px;background:var(--surface)}
.tc-h{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}
.tc-x{border:1px solid var(--line);background:var(--surface);color:var(--ink2);border-radius:8px;padding:5px 10px;font-size:.78rem;font-weight:600;cursor:pointer;flex-shrink:0}
.tc-x:hover{color:var(--red);border-color:var(--red)}
.tc-legs{display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin:11px 0}
.lc{display:inline-flex;align-items:center;gap:4px;background:var(--soft);border-radius:999px;padding:4px 9px;font-size:.76rem;font-weight:600;color:var(--ink2)}
.lc-ar{color:var(--ink3)}
.tc-status{font-size:.86rem;font-weight:600;padding:9px 11px;border-radius:10px;background:var(--soft);color:var(--ink2)}
.tc-status.ok{color:var(--green)}
.tc-status.warn{color:var(--amber);background:color-mix(in srgb,var(--amber) 12%,var(--surface))}
.tc-status.bad{color:var(--red);background:color-mix(in srgb,var(--red) 11%,var(--surface))}`;
  const body = `${nav()}<div class="wrap">
  <div class="th"><h1>🔔 My trips</h1><p class="muted">Save a journey and get a heads-up — the day before, the morning of, and just before you leave — even with the app closed.</p></div>
  <div class="card" style="margin-bottom:16px"><div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap"><div><b>Proactive alerts</b><div class="muted" id="alertstate" style="font-size:.85rem;margin-top:2px">Checking…</div></div><button class="btn blue" id="enableBtn">Enable alerts</button></div></div>
  <div id="trips"><div class="muted" style="padding:22px;text-align:center">Loading…</div></div>
  <div style="margin-top:18px"><a class="btn blue" href="/">＋ Plan &amp; track a new trip</a></div>
  ${footer()}
</div>`;
  const script = `
var trips=document.getElementById('trips');
function ec(s){return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}
function statePill(){var p=(window.Notification&&Notification.permission)||'default';var el=document.getElementById('alertstate'),btn=document.getElementById('enableBtn');
 if(p==='granted'){el.textContent='On — you\\'ll be alerted before each saved trip.';btn.textContent='Re-check this device'}
 else if(p==='denied'){el.textContent='Blocked — allow notifications for this site in your browser settings.';btn.textContent='How to enable'}
 else{el.textContent='Off — turn on day-before & departure alerts.';btn.textContent='Enable alerts'}}
document.getElementById('enableBtn').addEventListener('click',function(){enablePush(function(){statePill()})});
statePill();
var MI={train:'🚆',walking:'🚶',tube:'🚇',bus:'🚌','elizabeth-line':'🚆',dlr:'🚈',overground:'🚆',tram:'🚊'};
function legChip(l){var ic=MI[l.mode]||'🚆';var t=l.mode==='train'?((l.from||'')+' → '+(l.to||'')):(l.line||(l.mode.charAt(0).toUpperCase()+l.mode.slice(1).replace(/-/g,' ')));return '<span class="lc">'+ic+' '+ec(t)+'</span>'}
function card(t){return '<div class="tcard"><div class="tc-h"><div><b>'+ec(t.name||(t.origin+' → '+t.destination))+'</b><div class="muted" style="font-size:.84rem">'+ec(t.travel_date)+' · '+ec(t.depart||'')+' → '+ec(t.arrive||'')+'</div></div><button class="tc-x" data-id="'+t.id+'">Remove</button></div><div class="tc-legs">'+(t.legs||[]).map(legChip).join('<span class="lc-ar">→</span>')+'</div><div class="tc-status" id="st-'+t.id+'">Checking live status…</div></div>'}
function load(){fetch('/api/trips?owner='+encodeURIComponent(owner())).then(function(r){return r.json()}).then(function(d){
  var ts=d.trips||[];
  if(!ts.length){trips.innerHTML='<div class="muted" style="padding:26px;text-align:center">No saved trips yet. Plan a journey on the <a href="/" style="color:var(--blue);font-weight:600">home page</a> and tap <b>Track this trip</b>.</div>';return}
  trips.innerHTML=ts.map(card).join('');
  Array.prototype.forEach.call(document.querySelectorAll('.tc-x'),function(b){b.addEventListener('click',function(){if(!confirm('Remove this trip?'))return;fetch('/api/trips/'+b.dataset.id+'?owner='+encodeURIComponent(owner()),{method:'DELETE'}).then(load)})});
  ts.forEach(function(t){fetch('/api/trips/'+t.id+'/check',{method:'POST'}).then(function(r){return r.json()}).then(function(res){var el=document.getElementById('st-'+t.id);if(!el)return;el.className='tc-status '+(res.worst==='major'?'bad':res.worst==='minor'?'warn':'ok');el.innerHTML=(res.worst==='major'?'⚠️ ':res.worst==='minor'?'• ':'✅ ')+ec(res.summary||'')}).catch(function(){var el=document.getElementById('st-'+t.id);if(el)el.textContent='Live status unavailable.'})})
 }).catch(function(){trips.innerHTML='<div class="muted" style="padding:26px;text-align:center">Couldn\\'t load trips — the alerts backend may not be configured yet.</div>'})}
load();`;
  return shell({ title: 'My trips — Rail Tracker', desc: 'Save a rail journey and get proactive disruption alerts before you travel.', head, body, script });
}
...
module.exports = {
  ...
  renderOperators, renderOperator, renderAbout, renderTrips, render404, renderError,
  SW_JS, MANIFEST_JSON,
};
