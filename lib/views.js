...
function itinHTML(p,mode){var late=p.legs.some(function(l){return l.mode==='train'&&(l.depDelay>5||l.arrDelay>5)});
...
function pickTrain(i){var p=JP&&JP[i];if(!p)return;
 ...
 document.getElementById('itin').innerHTML=itinHTML(p,JM)}
function render(d){var plans=d.plans||[];var head=(d.fromName||fromCode)+' → '+(d.toName||toCode);
 ...
 html+='<div id="itin">'+itinHTML(plans[0],d.mode)+'</div></div>';
 res.innerHTML=html;
