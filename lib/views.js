...
.plan{border:1px solid var(--line);border-radius:16px;margin-bottom:10px;background:#fff;overflow:hidden}
...
.tp{flex:0 0 auto;display:flex;flex-direction:column;align-items:flex-start;gap:1px;padding:9px 13px;border:1px solid var(--line);border-radius:13px;background:#fff;cursor:pointer;min-width:92px;text-align:left}
...
.walkov{position:fixed;inset:0;z-index:1000;background:#fff;display:none;flex-direction:column}
...
.wk-x{width:34px;height:34px;border-radius:50%;border:1px solid var(--line);background:#fff;cursor:pointer;font-size:.95rem;flex-shrink:0;color:var(--ink)}
...
.buy{border:1px solid var(--line);border-radius:16px;margin:14px 0 6px;overflow:hidden;background:#fff}
...
function openWalk(pi,li){var p=JP&&JP[pi];if(!p)return;var l=p.legs[li];if(!l)return;
 ...
 wkMap=new maplibregl.Map({container:'wk-map',style:'${MAP_STYLE}',attributionControl:false});
...
<div class="train">${plan.coaches.map((c, i) => {
    const ends = (i === 0 ? ' head' : '') + (i === nc - 1 ? ' tail' : '');
    const fac = [c.access ? '♿' : '', c.bike ? '🚲' : '', c.lug ? '🧳' : '', c.cater ? '🍴' : ''].filter(Boolean).map((e) => `<span>${e}</span>`).join('');
    return `<div class="car${c.first ? ' first' : ''}${ends}"><div class="car-win"><i></i><i></i><i></i></div><div class="car-no">${i + 1}</div><div class="car-cls">${c.first ? '1ST' : 'STD'}</div><div class="car-fac">${fac}</div></div>`;
  }).join('')}</div>
...
.train{display:flex;align-items:stretch;gap:4px;overflow-x:auto;padding:8px 2px 12px}
.car{position:relative;flex:1 0 58px;min-width:58px;background:linear-gradient(180deg,#fff,#eceff4);border:1.5px solid var(--line);border-radius:9px;padding:9px 5px 8px;display:flex;flex-direction:column;align-items:center;gap:4px;box-shadow:0 1px 2px rgba(16,24,40,.06)}
.car.head{border-top-left-radius:28px;border-bottom-left-radius:28px}
.car.tail{border-top-right-radius:28px;border-bottom-right-radius:28px}
.car.first{background:linear-gradient(180deg,#FFFDF5,#F4E7BE);border-color:#E2CE92}
.car-win{display:flex;gap:3px;margin-bottom:1px}
.car-win i{width:7px;height:6px;border-radius:1.5px;background:#cdd4df}
.car.first .car-win i{background:#DEC67C}
.car-no{font-size:.85rem;font-weight:800;color:var(--ink);line-height:1}
.car-cls{font-size:.57rem;font-weight:800;letter-spacing:.05em;color:var(--ink3)}
.car.first .car-cls{color:#937619}
.car-fac{display:flex;flex-wrap:wrap;justify-content:center;gap:3px;min-height:15px}
.car-fac span{font-size:.78rem;line-height:1}
