...
const GESTURE_CSS = 'https://unpkg.com/leaflet-gesture-handling/dist/leaflet-gesture-handling.min.css';
const GESTURE_JS = 'https://unpkg.com/leaflet-gesture-handling/dist/leaflet-gesture-handling.min.js';
...
.land-nav{position:absolute;top:0;left:0;right:0;z-index:20;display:flex;align-items:center;gap:16px;padding:14px 22px}
...
.leaflet-gesture-handling-touch-warning,.leaflet-gesture-handling-scroll-warning{font-family:inherit}
...
@media(max-width:760px){th.opt,td.opt{display:none}.panel{border-radius:18px 18px 0 0}.land-nav .brand{display:none}}`;
  ...
  const script = `
[['${LEAFLET_CSS}'],['${GESTURE_CSS}']].forEach(function(u){var l=document.createElement('link');l.rel='stylesheet';l.href=u[0];document.head.appendChild(l)});
var sc=document.createElement('script');sc.src='${LEAFLET_JS}';sc.onload=function(){var g=document.createElement('script');g.src='${GESTURE_JS}';g.onload=load;g.onerror=load;document.body.appendChild(g)};document.body.appendChild(sc);
...
function load(){
 var map=L.map('map',{zoomControl:false,attributionControl:false,gestureHandling:true}).setView([54.3,-2.6],6);
 ...
 function refresh(){
  ...
  if(z>=8){var n=0;for(var i=0;i<ALL.length&&n<700;i++){var s=ALL[i];if(majorCodes[s.code]||!b.contains([s.lat,s.lon]))continue;n++;
   L.circleMarker([s.lat,s.lon],{renderer:canvas,radius:z>=11?4:3,color:'#fff',weight:1,fillColor:'#5B6472',fillOpacity:.95}).on('click',go(s.code)).bindTooltip(s.name+' ('+s.code+')',{direction:'top'}).addTo(secLayer);}}
 ...
 Promise.all([fetch('/api/disruptions').then(function(r){return r.json()}),fetch('/api/locations').then(function(r){return r.json()})]).then(function(res){
  ...
  var pts=[];
  d.forEach(function(s){if(s.lat==null)return;pts.push([s.lat,s.lon]);L.marker([s.lat,s.lon],{icon:dotIcon(s.color),keyboard:false,zIndexOffset:1000}).on('click',go(s.code)).addTo(map);});
