...
function legView(l){var col=legColor(l),ic=MODEIC[l.mode]||'🚆';
 ...
 var sub=l.mode==='walking'?(l.from?'from '+l.from:''):(l.line?(/line/i.test(l.line)?l.line:l.line+' line'):cap(l.mode));
...
function render(d){var plans=d.plans||[];var head=(d.fromName||fromCode)+' → '+(d.toName||toCode);
 ...
 var label=d.mode==='direct'?'Next direct trains':d.mode==='london'?'Across London':d.viaName?('via '+d.viaName):'Door to door';
 res.innerHTML='<div class="jres"><div class="jres-h"><span>'+head+'</span><span class="via">'+label+'</span></div>'+plans.map(function(p){
  var late=p.legs.some(function(l){return l.mode==='train'&&(l.depDelay>5||l.arrDelay>5)});
  var badge=p.changes===0?(d.mode==='direct'?'Direct':'No changes'):(p.changes+' change'+(p.changes>1?'s':''));
  return '<div class="plan"><div class="plan-head"><div class="plan-times"><span class="t'+(late?' late':'')+'">'+p.depart+'</span><span class="ar">→</span><span class="t'+(late?' late':'')+'">'+p.arrive+'</span></div>'
   +'<div class="plan-r"><div class="plan-dur">'+(p.duration!=null?fmtDur(p.duration):'')+'</div><div class="plan-meta">'+badge+'</div></div></div>'
   +'<div class="legs">'+p.legs.map(legView).join('')+'</div></div>';
 }).join('')+'</div>';
