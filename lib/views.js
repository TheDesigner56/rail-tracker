...
function load(){
 ...
 fetch('/api/disruptions').then(r=>r.json()).then(function(d){
  ...
  var pts=[];
  d.forEach(function(s){ if(s.lat==null)return; pts.push([s.lat,s.lon]);
   L.marker([s.lat,s.lon],{icon:dotIcon(s.color),keyboard:false}).on('click',function(){location.href='/station/'+s.code}).addTo(map);
  });
  if(pts.length)map.fitBounds(pts,{padding:[50,50]});
  var lyr=L.layerGroup().addTo(map);
  function labels(){ lyr.clearLayers(); var placed=[];
   d.forEach(function(s){ if(s.lat==null)return; var p=map.latLngToLayerPoint([s.lat,s.lon]);
    if(placed.some(function(q){return Math.abs(q.x-p.x)<54 && Math.abs(q.y-p.y)<22}))return;
    placed.push(p);
    L.tooltip({permanent:true,direction:'bottom',offset:[0,11],className:'pinlab',interactive:false}).setLatLng([s.lat,s.lon]).setContent(s.code).addTo(lyr);
   });}
  labels(); map.on('zoomend moveend',labels);
