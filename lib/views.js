...
.sh-card{pointer-events:auto;background:rgba(255,255,255,.97);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);border-radius:18px;box-shadow:0 16px 46px rgba(0,0,0,.22);padding:18px;width:100%;max-width:470px}
...
.ac{position:absolute;top:calc(100% + 4px);left:0;right:0;background:#fff;border:1px solid var(--line);border-radius:12px;box-shadow:0 10px 26px rgba(0,0,0,.16);z-index:30;max-height:280px;overflow-y:auto;display:none;padding:4px}
...
.panel{position:relative;z-index:5;background:#fff;border-radius:26px 26px 0 0;margin-top:-34px;padding:28px 0 8px;box-shadow:0 -10px 30px rgba(0,0,0,.06)}
...
@media(max-width:560px){.sh-title{font-size:1.35rem}.j-times{font-size:1.05rem}#map{height:60vh}}`;
  const body = `<div class="land">
  <div id="map"></div>
  <header class="land-nav">
    <a href="/" class="brand"><span class="mark">🚆</span>Rail<span class="sub">Tracker</span></a>
    <button class="tv" onclick="tvMode()" style="margin-left:auto">🖥 TV Mode</button>
  </header>
...
function initMap(){
 map=new maplibregl.Map({container:'map',style:'${MAP_STYLE}',center:[-2.4,54.4],zoom:5.2,attributionControl:false,cooperativeGestures:true});
...
function render(d){
 if(!d.services||!d.services.length){res.innerHTML='<div class="jres"><div class="jres-h">'+(d.fromName||fromCode)+' → '+(d.toName||toCode)+'</div><p class="muted" style="padding:2px">No direct trains in the next couple of hours — you may need to change. Try the <a href="/station/'+fromCode+'/departures" style="color:var(--blue);font-weight:600">'+(d.fromName||fromCode)+' departures board</a>.</p></div>'}
 else{document.getElementById('from-input').value=d.fromName||fromCode;document.getElementById('to-input').value=d.toName||toCode;
  res.innerHTML='<div class="jres"><div class="jres-h">'+d.fromName+' → '+d.toName+' · next direct trains</div>'+d.services.map(function(s){
   var cls=s.cancelled?'bad':(s.depDelay>5||s.arrDelay>5)?'late':'';var stops=s.intermediateStops===0?'Non-stop':(s.intermediateStops+' stop'+(s.intermediateStops>1?'s':''));
   return '<a class="jrow" href="/service/'+s.id+'"><div class="j-l"><div class="j-times"><span class="t '+cls+'">'+s.dep+'</span><span class="ar">→</span><span class="t '+cls+'">'+s.arr+'</span></div><div class="j-sub"><span class="oplogo" style="background:'+s.color+'">'+s.operatorCode+'</span> '+s.operator+' · Plat '+(s.platform||'—')+' · '+stops+(s.cancelled?' · <b class="red">Cancelled</b>':'')+'</div></div><div class="j-r"><div class="j-dur">'+(s.duration!=null?fmtDur(s.duration):'')+'</div><div class="j-badge">Direct</div></div></a>';
  }).join('')+'</div>'}
...
function search(){if(!fromCode||!toCode){res.innerHTML='<p class="muted" style="padding:6px 2px">Choose where you\\'re travelling from and to.</p>';return}
 ...
 fetch('/api/journey?from='+fromCode+'&to='+toCode).then(function(r){return r.json()}).then(render).catch(function(){res.innerHTML='<p class="muted" style="padding:6px 2px">Couldn\\'t plan that journey. Please try again.</p>'})}
