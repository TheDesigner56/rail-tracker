...
:root{--bg:#fff;--soft:#F5F5F7;--soft2:#FAFAFA;--line:#ECECEF;--ink:#16181D;--ink2:#6B7280;--ink3:#9AA0AA;--blue:#2563EB;--green:#16A34A;--amber:#F59E0B;--red:#EF4444}
...
.search input{width:100%;background:var(--soft);border:1px solid transparent;border-radius:999px;padding:9px 16px 9px 38px;font-size:.92rem;color:var(--ink);outline:none}
...
.stacked{display:flex;height:8px;border-radius:5px;overflow:hidden;background:var(--soft);margin-top:4px}
...
function delayCard(title, icon, st, extra = '') {
  return `<div class="card">
    ...
    <div style="display:flex;gap:8px">
      ${[['On time', st.onTimePct, st.onTime, 'green'], ['Delayed', st.delayedPct, st.delayed, 'red'], ['Canceled', st.cancelPct, st.cancelled, 'red']].concat(extra ? [['Diverted', 0, 0, 'red']] : []).map(([l, p, n, c]) =>
        `<div style="flex:1"><div style="font-size:.74rem;color:var(--ink2);display:flex;align-items:center;gap:5px"><span class="dot bg-${c === 'green' ? 'green' : l === 'Delayed' ? 'amber' : 'red'}"></span>${l}</div><div style="font-size:1.7rem;font-weight:800;line-height:1.1;margin-top:3px">${p}%</div><div style="font-size:.78rem;color:var(--ink3)">${n}</div></div>`).join('')}
    </div>${stacked(st)}</div>`;
}
function barList(items, max) {
  ...
  return items.map((i) => `<div style="display:flex;align-items:center;gap:10px;margin:9px 0">
    ...
    <div style="flex:1;height:9px;border-radius:5px;background:var(--soft);overflow:hidden"><span style="display:block;height:100%;width:${(i.value / m) * 100}%;background:${i.color || 'var(--red)'};border-radius:5px"></span></div>
...
function renderDisruptions() {
  const head = `
#map{height:54vh;min-height:380px;width:100%;background:#AAD3DF}
.leaflet-container{font-family:inherit}
.leaflet-tooltip.pin{background:#fff;border:1px solid var(--line);border-radius:8px;box-shadow:0 2px 6px rgba(0,0,0,.15);font-weight:700;font-size:.72rem;padding:2px 7px;color:var(--ink)}
.leaflet-tooltip.pin::before{display:none}
.panel{max-width:1180px;margin:-46px auto 0;position:relative;background:#fff;border-radius:24px 24px 0 0;box-shadow:0 -8px 32px rgba(0,0,0,.08);padding:26px 20px 8px}
...
.toggle{display:inline-flex;background:var(--soft);border-radius:999px;padding:3px}
.toggle span{padding:6px 14px;border-radius:999px;font-size:.82rem;font-weight:600;color:var(--ink2)}
.toggle span.on{background:#fff;color:var(--ink);box-shadow:0 1px 3px rgba(0,0,0,.1)}
.region{font-size:1rem;font-weight:700;color:var(--ink2);margin-top:2px}
table{width:100%;border-collapse:collapse}
th{text-align:left;font-size:.7rem;text-transform:uppercase;letter-spacing:.05em;color:var(--ink3);font-weight:700;padding:10px 12px;border-bottom:1px solid var(--line)}
...
.d-metric{display:flex;align-items:center;gap:12px}
.d-metric .big{font-weight:700;min-width:42px}
.d-metric .sub{display:flex;align-items:center;gap:4px;color:var(--ink2);font-size:.82rem}
.d-alerts{color:var(--red);font-weight:600;font-size:.82rem}
.loading{padding:50px;text-align:center;color:var(--ink3)}
@media(max-width:760px){th.opt,td.opt{display:none}.panel{border-radius:18px 18px 0 0}}`;
  const body = `<div style="position:relative">${nav()}<div id="map"></div>
<div class="panel">
  <div class="panel-top">
    <div><h1>Major Stations &amp; Disruptions</h1><div class="region">Great Britain ▾</div></div>
    <div class="toggle"><span class="on">● Live</span><span>Today</span></div>
  </div>
...
var sc=document.createElement('script');sc.src='${LEAFLET_JS}';sc.onload=load;document.body.appendChild(sc);
function metric(m){return '<div class="d-metric"><span class="big">'+(m.avgDelay>=60?(Math.floor(m.avgDelay/60)+'h '+(m.avgDelay%60)+'m'):m.avgDelay+'m')+'</span><span class="sub">✓ '+m.onTimePct+'%</span><span class="sub">✕ '+m.cancelPct+'%</span></div>'}
function load(){
 var map=L.map('map',{zoomControl:true,attributionControl:false,scrollWheelZoom:false}).setView([54.3,-2.4],5);
 L.tileLayer('${LIGHT_TILES}',{maxZoom:18,subdomains:'abcd'}).addTo(map);
 fetch('/api/disruptions').then(r=>r.json()).then(function(d){
  var rows=document.getElementById('rows');
  rows.innerHTML=d.map(function(s){return '<tr onclick="location.href=\\'/station/'+s.code+'\\'" style="cursor:pointer">'+
   '<td><span class="d-code"><span class="dot" style="background:'+s.color+'"></span>'+s.code+'</span></td>'+
   '<td class="opt muted">'+(s.city||'')+'</td>'+
   '<td>'+metric(s.dep)+'</td>'+
   '<td class="opt">'+metric(s.arr)+'</td>'+
  ...
  d.forEach(function(s){ if(s.lat==null)return;
   L.circleMarker([s.lat,s.lon],{radius:7,color:'#fff',weight:2,fillColor:s.color,fillOpacity:1}).addTo(map)
    .bindTooltip(s.code,{permanent:true,direction:'top',className:'pin',offset:[0,-4]})
    .on('click',function(){location.href='/station/'+s.code});
  });
...
function renderOverview(o) {
  ...
  const weatherCard = o.weather ? `<div class="card" id="wx">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px"><b style="font-size:1rem">🌡 Current Weather</b>
      <span class="toggle2"><span data-u="m" class="on">Metric</span><span data-u="i">Imperial</span></span></div>
    ...
    <div style="display:flex;gap:10px;margin-top:18px">
      <div style="flex:1;background:var(--soft);border-radius:12px;padding:11px"><div style="font-size:.72rem;color:var(--ink2)">💨 Wind</div><div id="wx-wind" style="font-size:1.15rem;font-weight:700" data-k="${o.weather.windKmh}">${o.weather.windKmh} km/h</div></div>
      <div style="flex:1;background:var(--soft);border-radius:12px;padding:11px"><div style="font-size:.72rem;color:var(--ink2)">🌬 Gusts</div><div id="wx-gust" style="font-size:1.15rem;font-weight:700" data-k="${o.weather.gustKmh}">${o.weather.gustKmh} km/h</div></div>
    </div></div>`
...
.toggle2{display:inline-flex;background:var(--soft);border-radius:999px;padding:3px;font-size:.76rem}
.toggle2 span{padding:4px 10px;border-radius:999px;font-weight:600;color:var(--ink2);cursor:pointer}
.toggle2 span.on{background:#fff;color:var(--ink);box-shadow:0 1px 2px rgba(0,0,0,.12)}
...
<div class="tabs"><a href="/station/${o.crs}" class="on">Overview</a><a href="/station/${o.crs}/departures">Departures</a><a href="/station/${o.crs}/arrivals">Arrivals</a>
  <span class="acts"><button class="btn" onclick="navigator.share?navigator.share({title:'${esc(o.name)}',url:location.href}):navigator.clipboard.writeText(location.href)">⤴ Share</button></span></div>
...
<div class="grid g3" style="margin-bottom:16px">
  <div class="status-card" style="background:${statusTint};border-color:${statusBorder}">
    ...
    <div class="ln"><span>✈️</span><span><b>Arrivals &amp; Departures</b><span class="d">${st.alerts.length ? st.alerts.join(' · ') : 'No operational issues reported.'}</span></span></div>
  ...
  ${delayCard('Departures', '🛫', o.depStats)}
  ${delayCard('Arrivals', '🛬', o.arrStats)}
</div>

<div class="grid g2w" style="margin-bottom:16px">
  ${weatherCard}
  <div class="card">
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px"><span>📊</span><b style="font-size:1rem">Daily Performance</b></div>
    <div style="display:flex;align-items:flex-end;gap:18px;flex-wrap:wrap">
      <div><div class="big-num">${o.depStats.total}</div><div class="muted" style="font-size:.82rem">Departures</div></div>
      <div style="flex:1;min-width:160px">
        <div class="kv"><span class="lab"><span class="dot bg-green"></span>On time</span><span><b>${o.depStats.onTimePct}%</b> <span class="muted">${o.depStats.onTime}</span></span></div>
        <div class="kv"><span class="lab"><span class="dot bg-amber"></span>Delayed</span><span><b>${o.depStats.delayedPct}%</b> <span class="muted">${o.depStats.delayed}</span></span></div>
        <div class="kv"><span class="lab"><span class="dot bg-red"></span>Canceled</span><span><b>${o.depStats.cancelPct}%</b> <span class="muted">${o.depStats.cancelled}</span></span></div>
      </div>
    </div>${stacked(o.depStats)}
...
<div class="grid g2" style="margin-bottom:16px">
  <div class="card"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px"><b style="font-size:1.05rem">Departures Board</b><a class="view-all" href="/station/${o.crs}/departures">View all →</a></div>
    ${o.dep.services.slice(0, 7).map((s) => boardRow(s, false)).join('') || '<div class="muted" style="padding:18px 0">No departures.</div>'}</div>
  <div class="card"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px"><b style="font-size:1.05rem">Arrivals Board</b><a class="view-all" href="/station/${o.crs}/arrivals">View all →</a></div>
    ${o.arr.services.slice(0, 7).map((s) => boardRow(s, true)).join('') || '<div class="muted" style="padding:18px 0">No arrivals.</div>'}</div>
</div>

<div class="grid g2">
  <div class="card"><div style="display:flex;align-items:center;gap:8px;margin-bottom:14px"><span>🧭</span><b style="font-size:1rem">Station Stats</b></div>
  ...
  <div class="card" style="display:flex;flex-direction:column;justify-content:center;align-items:flex-start;background:var(--soft2)">
...
${footer()}</div>`;
  ...
  const script = `(function(){var tg=document.querySelectorAll('#wx .toggle2 span');if(!tg.length)return;
...
#map{height:300px;border-radius:18px;overflow:hidden;border:1px solid var(--line);margin:6px 0 16px}
.leaflet-container{background:#dfeaf0!important;font-family:inherit}
...
.sect-title{margin:4px 0 8px}`;
  const body = `${nav()}<div class="wrap">
<div class="svc-hero"><div class="svc-route">${esc(svc.origin || '?')}<span class="to">→</span>${esc(svc.destination || '?')}</div>
...
var D=${mapData},map,line,tM;
function initMap(){map=L.map('map',{zoomControl:false,attributionControl:false,preferCanvas:true});
 L.tileLayer('${LIGHT_TILES}',{maxZoom:18,subdomains:'abcd'}).addTo(map);
 if(D.points&&D.points.length){line=L.polyline(D.points,{color:D.color,weight:4,opacity:.85}).addTo(map);
  L.circleMarker(D.points[0],{radius:5,color:'#fff',fillColor:D.color,fillOpacity:1,weight:2}).addTo(map);
  L.circleMarker(D.points[D.points.length-1],{radius:5,color:'#fff',fillColor:'#16181D',fillOpacity:1,weight:2}).addTo(map);
  map.fitBounds(line.getBounds().pad(.18))}else{map.setView([54,-2],5)}
 draw(D.pos);setInterval(poll,20000)}
function draw(p){if(!p)return;if(tM)tM.setLatLng(p);else tM=L.circleMarker(p,{radius:8,color:'#fff',fillColor:D.color,fillOpacity:1,weight:3}).addTo(map)}
function poll(){fetch('/api/service/${esc(svc.id)}?date=${esc(svc.date)}').then(r=>r.json()).then(function(d){
 ...
 if(d.currentPosition)draw(d.currentPosition);
 if(d.stops){var tl=document.getElementById('timeline');d.stops.forEach(function(s){var el=tl.querySelector('[data-name="'+(s.name||'').replace(/"/g,'')+'"]');if(el){el.classList.remove('past','cur','fut');el.classList.add(s.state==='current'?'cur':s.state==='past'?'past':'fut')}})}}).catch(function(){})}`;
...
// ── Network map ────────────────────────────────────────────────────────────
function renderMap() {
  const head = `#map{position:fixed;top:54px;left:0;right:0;bottom:0}.leaflet-container{font-family:inherit}
.leaflet-popup-content a{color:var(--blue);font-weight:700}.nav{position:fixed;left:0;right:0;top:0}`;
  const body = `${nav()}<div id="map"></div>`;
  const script = `var lk=document.createElement('link');lk.rel='stylesheet';lk.href='${LEAFLET_CSS}';document.head.appendChild(lk);
...
function init(){var p=new URLSearchParams(location.search),f=p.get('focus');
 var map=L.map('map',{attributionControl:false,preferCanvas:true}).setView([54.3,-2.4],6);
 L.tileLayer('${LIGHT_TILES}',{maxZoom:18,subdomains:'abcd'}).addTo(map);
 fetch('/api/locations').then(r=>r.json()).then(function(list){var fm;list.forEach(function(s){
  var m=L.circleMarker([s.lat,s.lon],{radius:3,color:'#2563EB',fillColor:'#60A5FA',fillOpacity:.85,weight:1});
  m.bindPopup('<b>'+s.name+'</b> ('+s.code+')<br><a href="/station/'+s.code+'">Live times →</a>');m.addTo(map);
  if(f&&s.code===f)fm=s});if(fm)map.setView([fm.lat,fm.lon],13)})}`;
