...
#map{height:58vh;min-height:440px;width:100%;background:#A8D5E6;position:relative;z-index:1}
.leaflet-container{font-family:inherit;background:#A8D5E6}
...
.pin-ic{overflow:visible}
.pin-dot{position:absolute;left:1px;top:1px;width:18px;height:18px;border-radius:50%;border:2.5px solid #fff;box-sizing:border-box;box-shadow:0 1px 4px rgba(0,0,0,.45)}
.pin-radar{position:absolute;left:50%;top:50%;width:34px;height:34px;border-radius:50%;transform:translate(-50%,-50%);opacity:.16;pointer-events:none}
.pin-ic:hover{z-index:10000!important}
.pin-ic:hover .pin-radar{animation:radar 1.5s ease-out infinite}
@keyframes radar{0%{opacity:.45;width:18px;height:18px}100%{opacity:0;width:66px;height:66px}}
.leaflet-tooltip.pinlab{background:#fff;border:none;border-radius:8px;box-shadow:0 2px 7px rgba(0,0,0,.28);font-weight:700;font-size:.72rem;padding:2px 8px;color:var(--ink);margin:0}
.leaflet-tooltip.pinlab::before{display:none}
.leaflet-tooltip.pinlab.sec{color:var(--ink2);font-weight:600;box-shadow:0 1px 4px rgba(0,0,0,.18)}
...
.leaflet-tooltip.stncard{background:#fff;border:none;border-radius:13px;box-shadow:0 8px 26px rgba(0,0,0,.22);padding:11px 13px;color:var(--ink);font-size:.8rem;min-width:190px;white-space:normal;text-align:left;font-weight:400}
.leaflet-tooltip.stncard::before{display:none}
.stncard .sc-h{display:flex;align-items:center;gap:7px;font-weight:700;margin-bottom:8px;font-size:.86rem}
.stncard .sc-h .dot{width:9px;height:9px;border-radius:50%}
.stncard .sc-row{display:flex;justify-content:space-between;gap:12px;margin:4px 0}
.stncard .sc-row .muted{color:var(--ink2)}
.stncard .sc-alert{color:var(--red);font-weight:600;margin-top:7px;font-size:.76rem}
.stncard .sc-go{color:var(--blue);font-weight:600;margin-top:7px;font-size:.76rem}
...
function dotIcon(c){return L.divIcon({className:'pin-ic',iconSize:[20,20],iconAnchor:[10,10],html:'<span class="pin-radar" style="background:'+c+'"></span><span class="pin-dot" style="background:'+c+'"></span>'})}
function card(s){return '<div class="sc-h"><span class="dot" style="background:'+s.color+'"></span><b>'+s.code+'</b> '+s.name+'</div>'+
 '<div class="sc-row"><span>🚆 Departures</span><span><b>'+fmt(s.dep.avgDelay)+'</b> <span class="muted">'+s.dep.onTimePct+'% on time</span></span></div>'+
 '<div class="sc-row"><span>🚉 Arrivals</span><span><b>'+fmt(s.arr.avgDelay)+'</b> <span class="muted">'+s.arr.onTimePct+'% on time</span></span></div>'+
 (s.alerts&&s.alerts.length?'<div class="sc-alert">⚠ '+s.alerts.join(' • ')+'</div>':'<div class="sc-go">View live board →</div>')}
function load(){
 var map=L.map('map',{zoomControl:false,attributionControl:false}).setView([54.3,-2.6],6);
 ...
 var canvas=L.canvas({padding:.5}),secLayer=L.layerGroup().addTo(map),labelLayer=L.layerGroup().addTo(map);
 var MAJOR=[],ALL=[],majorCodes={};
 function go(c){return function(){location.href='/station/'+c}}
 function refresh(){
  var z=map.getZoom(),b=map.getBounds();
  // Secondary stations fade in as you zoom past the major hubs.
  secLayer.clearLayers();
  if(z>=8){var n=0;for(var i=0;i<ALL.length&&n<700;i++){var s=ALL[i];if(majorCodes[s.code]||!b.contains([s.lat,s.lon]))continue;n++;
   L.circleMarker([s.lat,s.lon],{renderer:canvas,radius:z>=11?5:4,color:'#fff',weight:1.5,fillColor:'#5B6472',fillOpacity:.95}).on('click',go(s.code)).bindTooltip('<div class="sc-h" style="margin-bottom:3px"><b>'+s.code+'</b> '+s.name+'</div><div class="sc-go">View live times →</div>',{direction:'top',className:'stncard',offset:[0,-4]}).addTo(secLayer);}}
  // Labels: major first (priority), then secondary at higher zoom, collision-avoided.
  labelLayer.clearLayers();var placed=[];
  function lab(s,cls,off){if(s.lat==null)return;var p=map.latLngToLayerPoint([s.lat,s.lon]);if(placed.some(function(q){return Math.abs(q.x-p.x)<54&&Math.abs(q.y-p.y)<22}))return;placed.push(p);L.tooltip({permanent:true,direction:'bottom',offset:[0,off],className:cls,interactive:false}).setLatLng([s.lat,s.lon]).setContent(s.code).addTo(labelLayer);}
  MAJOR.forEach(function(s){lab(s,'pinlab',12)});
  if(z>=10){var m=0;for(var j=0;j<ALL.length&&m<400;j++){var s2=ALL[j];if(majorCodes[s2.code]||!b.contains([s2.lat,s2.lon]))continue;m++;lab(s2,'pinlab sec',8)}}
 }
 Promise.all([fetch('/api/disruptions').then(function(r){return r.json()}),fetch('/api/locations').then(function(r){return r.json()})]).then(function(res){
  var d=res[0]||[];MAJOR=d;ALL=res[1]||[];d.forEach(function(s){majorCodes[s.code]=1});
  document.getElementById('rows').innerHTML=d.map(function(s){return '<tr onclick="location.href=\\'/station/'+s.code+'\\'" style="cursor:pointer">'+
  ...
  var pts=[];
  d.forEach(function(s){if(s.lat==null)return;pts.push([s.lat,s.lon]);L.marker([s.lat,s.lon],{icon:dotIcon(s.color),keyboard:false,zIndexOffset:1000}).bindTooltip(card(s),{direction:'top',className:'stncard',offset:[0,-10],opacity:1}).on('click',go(s.code)).addTo(map);});
  if(pts.length)map.fitBounds(pts,{padding:[50,50]});
  refresh();map.on('zoomend moveend',refresh);
...
.svc-actions{display:flex;gap:9px;margin-top:12px;flex-wrap:wrap}
...
#map{height:320px;border-radius:18px;overflow:hidden;border:1px solid var(--line);margin:6px 0 16px}
...
<div class="svc-hero"><div class="svc-route">${esc(svc.origin || '?')}<span class="to">→</span>${esc(svc.destination || '?')}</div>
  <div class="svc-sub">${opLogo(svc.operatorCode, svc.color)}<b>${esc(svc.operator || 'Unknown operator')}</b><span class="svc-status ${statusCls}" id="status">${esc(svc.status)}${svc.currentStation ? ` · ${esc(svc.currentStation)}` : ''}</span></div>
...
var D=${mapData},map,line,tM,LIVE=D.live;
...
function makeMk(p){tM=L.marker(p,{icon:L.divIcon({className:'',iconSize:[28,28],iconAnchor:[14,14],html:'<div class="trainmk" style="--c:'+D.color+'"><span class="tm-pulse"></span><span class="tm-dot">🚆</span></div>'}),zIndexOffset:1000,interactive:false}).addTo(map)}
function tick(){var p=trainPos();if(p){if(tM)tM.setLatLng(p);else makeMk(p)}requestAnimationFrame(tick)}
function initMap(){map=L.map('map',{zoomControl:false,attributionControl:false});
