...
async function getJourney(from, to) {
  ...
  }))).filter(Boolean).sort((a, b) => (parseHHMM(a.dep) || 0) - (parseHHMM(b.dep) || 0));
...
async function getJourneyPlan(from, to) {
  ...
  // 3) Regional → London: train to a terminus, then TfL across town.
...
module.exports = {
  ...
  getBoard, getService, getOverview, getDisruptions, getWeather, getJourney, getJourneyPlan,
