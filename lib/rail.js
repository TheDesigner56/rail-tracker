...
function delayTimeline(services) {
  const pts = (services || [])
    ...
    .map((s) => ({ m: parseHHMM(s.scheduled), d: Math.max(0, s.delay || 0) }))
    .filter((p) => p.m != null)
    .sort((a, b) => a.m - b.m);
  ...
  for (let i = 0; i < N; i++) {
    ...
    buckets.push({ avg: inB.length ? Math.round(inB.reduce((s, p) => s + p.d, 0) / inB.length) : 0, n: inB.length });
  }
...
async function getJourneyPlan(from, to) {
  ...
  if (isLondon(from) && isLondon(to)) {
    ...
    if (t) {
      ...
      return { ...meta, mode: 'london', plans: [{ kind: 'london', via: null, ...planTotals(legs), legs }] };
    }
  }
  ...
  if (toLoc && isLondon(to) && !isLondon(from)) {
    ...
    for (const terminus of terminiOnBoard(board).slice(0, 2)) {
      ...
      if (!best || earliest < best.earliest) best = { terminus, plans, earliest };
    }
    if (best) return { ...meta, mode: 'via', via: best.terminus, viaName: stationName(best.terminus), plans: best.plans };
  }
  ...
  if (fromLoc && isLondon(from) && !isLondon(to)) {
    ...
    for (const terminus of terminiOnBoard(board).slice(0, 2)) {
      ...
      return { ...meta, mode: 'via', via: terminus, viaName: stationName(terminus), plans };
    }
  }
...
module.exports = {
  ...
  getBoard, getService, getOverview, getDisruptions, getWeather, getJourney, getJourneyPlan, getTrainlineUrl,
