  ...
  for (let i = 0; i < tries; i++) {
    try {
      ...
      if (res.ok) {
        ...
        // A bot-challenge page returns 200 with no data — surface it as an
        // upstream failure rather than silently parsing zero services.
        if (text.includes('Just a moment') && /challenge/i.test(text)) throw new Error('upstream blocked (bot challenge)');
        return text;
      }
...
// ── Official Realtime Trains API (api.rtt.io) ───────────────────────────────
// The website now sits behind a bot challenge, so scraping fails from servers.
// With free credentials from https://api.rtt.io (env: RTT_API_USER +
// RTT_API_PASS) we use the official JSON API instead — same data, contracted
// and stable. Without credentials we fall back to the scraper.
const rttApiOn = () => !!(process.env.RTT_API_USER && process.env.RTT_API_PASS);
async function rttApi(path, tries = 3) {
  const auth = 'Basic ' + Buffer.from(`${process.env.RTT_API_USER}:${process.env.RTT_API_PASS}`).toString('base64');
  let lastErr;
  for (let i = 0; i < tries; i++) {
    try {
      const r = await fetch(`https://api.rtt.io/api/v1/json${path}`, { headers: { Authorization: auth, 'User-Agent': 'rail-tracker (+https://railtracker.uk)' } });
      if (r.ok) return r.json();
      if (r.status >= 500 || r.status === 429) lastErr = new Error(`rtt api ${r.status}`);
      else throw new Error(`rtt api ${r.status}`);
    } catch (e) { lastErr = e; }
    if (i < tries - 1) await new Promise((res) => setTimeout(res, 250 * (i + 1)));
  }
  throw lastErr || new Error('rtt api error');
}
const dSlash = (date) => date.replace(/-/g, '/');

async function apiBoard(crs, mode, at) {
  const date = today();
  const time = (typeof at === 'string' && /^\d{3,4}$/.test(at)) ? at.padStart(4, '0') : nowHHMM();
  const arr = mode === 'arrivals';
  const j = await rttApi(`/search/${crs}/${dSlash(date)}/${time}${arr ? '/arrivals' : ''}`);
  const services = [];
  for (const s of (j.services || [])) {
    if (s.isPassenger === false) continue;
    const ld = s.locationDetail || {};
    const sched = arr ? (ld.gbttBookedArrival || ld.wttBookedArrival) : (ld.gbttBookedDeparture || ld.wttBookedDeparture);
    if (!sched) continue; // terminates here (departures) / starts here (arrivals)
    const real = arr ? ld.realtimeArrival : ld.realtimeDeparture;
    const actual = arr ? !!ld.realtimeArrivalActual : !!ld.realtimeDepartureActual;
    const cancelled = /CANCELLED/.test(ld.displayAs || '') || !!ld.cancelReasonCode;
    const ends = arr ? ld.origin : ld.destination;
    const place = (ends && ends[0] && ends[0].description) || '';
    services.push({
      id: s.serviceUid, headcode: s.trainIdentity || '', operatorCode: s.atocCode,
      operator: s.atocName || operatorName(s.atocCode), color: operatorColor(s.atocCode),
      cars: null, platform: ld.platform || null, place,
      scheduled: fmtMins(sched), expected: fmtMins(real || sched),
      delay: real ? delayMins(sched, real) : 0, actual, cancelled,
    });
  }
  return annotateRisk({
...
async function apiService(id, date) {
  const j = await rttApi(`/service/${id}/${dSlash(date)}`);
  const route = [];
  for (const l of (j.locations || [])) {
    const code = l.crs || null;
    const loc = code && LOCATIONS[code] ? LOCATIONS[code] : null;
    const cancelled = /CANCELLED/.test(l.displayAs || '');
    const schedArr = l.gbttBookedArrival || l.wttBookedArrival, schedDep = l.gbttBookedDeparture || l.wttBookedDeparture;
    const realArr = l.realtimeArrival, realDep = l.realtimeDeparture;
    const late = (realDep && schedDep) ? delayMins(schedDep, realDep) : (realArr && schedArr) ? delayMins(schedArr, realArr) : 0;
    route.push({
      name: l.description || code || '?', code, platform: l.platform || null,
      stopsHere: l.isCall !== false && !/PASS/i.test(l.displayAs || ''),
      public: l.isPublicCall !== false && !!(l.gbttBookedArrival || l.gbttBookedDeparture),
      location: loc ? { lat: loc.lat, lon: loc.lon } : null,
      schedArr: fmtMins(schedArr), schedDep: fmtMins(schedDep),
      realArr: fmtMins(realArr), realDep: fmtMins(realDep),
      arrived: !!l.realtimeArrivalActual, departed: !!l.realtimeDepartureActual,
      delay: late, cancelled,
    });
  }
  const statusHint = (j.locations || []).some((l) => /CANCELLED/.test(l.displayAs || '')) ? 'Cancelled' : null;
  const train = { cars: null, formation: null, trainClass: null, power: null, maxSpeed: null, seating: null, reservations: null };
  return assembleService({
    id, date, headcode: j.trainIdentity || null,
    operator: j.atocName || operatorName(j.atocCode), operatorCode: j.atocCode,
    formation: null, tags: j.serviceType === 'train' ? [] : [j.serviceType].filter(Boolean), pathInfo: null, train,
  }, route, statusHint);
}
...
async function getBoard(crs, mode = 'departures', at) {
  ...
  if (rttApiOn()) return apiBoard(crs, mode, at);
  return scrapeBoard(crs, mode, at);
}
async function scrapeBoard(crs, mode = 'departures', at) {
  crs = crs.toUpperCase().slice(0, 3);
  const date = today();
  const time = (typeof at === 'string' && /^\d{3,4}$/.test(at)) ? at.padStart(4, '0') : nowHHMM();
...
async function getService(id, date) {
  ...
  if (rttApiOn()) return apiService(id, date);
  return scrapeService(id, date);
}
async function scrapeService(id, date) {
  ...
  try {
    ...
    if (cs && cs.body && cs.body.status) statusHint = cs.body.status;
  ...
  return assembleService({ id, date, headcode, operator, operatorCode, formation, tags, pathInfo, train }, route, statusHint);
}

// Shared tail for both data paths (scrape + official API): derive the live
// frontier, per-stop display state, positions and the interpolation segment.
function assembleService(base, route, statusHint) {
  let currentIndex = -1;
  ...
  let status = statusHint || 'Scheduled';
  if (status === 'Scheduled') {
    const last = route[route.length - 1];
    if (currentIndex === -1) status = 'Not yet departed';
    else if (last && last.arrived) status = 'Arrived';
  ...
  return {
    ...base, color: operatorColor(base.operatorCode),
    ...
    status, currentIndex, currentStation, currentDelay,
...
async function getJourney(from, to) {
  ...
  const base = [];
  if (rttApiOn()) {
    const j = await rttApi(`/search/${from}/to/${to}/${dSlash(date)}/${time}`);
    for (const s of (j.services || [])) {
      if (s.isPassenger === false) continue;
      base.push({ id: s.serviceUid, operatorCode: s.atocCode, operator: s.atocName || operatorName(s.atocCode), color: operatorColor(s.atocCode), headcode: s.trainIdentity || '' });
