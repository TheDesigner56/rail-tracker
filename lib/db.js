...
function sb() {
  ...
  // The project URL isn't secret, so default it — only the service-role key
  // (a real secret) must be supplied via an env var.
  const url = process.env.SUPABASE_URL || 'https://qjlwnrfuirgsegupaagu.supabase.co';
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY;
  _sb = key ? createClient(url, key, { auth: { persistSession: false } }) : null;
  return _sb;
}
const configured = () => !!sb();
